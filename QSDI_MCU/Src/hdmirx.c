...
//    #pragma message("OUTPUT_SYNC_EMBEDDED defined")
  ...
  //  #pragma message("OUTPUT_24BIT_YUV422 defined")
    #define HDMIRX_OUTPUT_MAPPING   0//0 
    #define HDMIRX_OUTPUT_TYPE  B_SYNC_EMBEDDED
    #define HDMIRX_OUTPUT_COLORMODE (B_OUTPUT_YUV422<<O_OUTPUT_COLOR_MODE)
...
#endif
//static BOOL bGetSyncInfovoid(void);
...
static void SetColorimetryByMode(void);
...
BYTE HDMIRX_ReadI2C_Byte(BYTE RegAddr)
{
	...
	//return 0;
 HAL_I2C_Mem_Read(&hi2c2, HDMI_RX_I2C_SLAVE_ADDR, RegAddr, 1, &p_data, 1, 10);
delay1ms(20);
  //  p_data=ReadI2C_Byte(HDMI_RX_I2C_SLAVE_ADDR, RegAddr);

    return p_data;

}

void  HDMIRX_WriteI2C_Byte(uint8_t RegAddr,uint8_t p_data)
{
 //BYTE p_data;
HAL_I2C_Mem_Write(&hi2c2,HDMI_RX_I2C_SLAVE_ADDR,RegAddr,I2C_MEMADD_SIZE_8BIT,&p_data,1,10);
...
//USART1_printf( USART1," p_data %x    \r\n",p_data);
}


void  HDMIRX_WriteI2C_ByteN(uint16_t RegAddr,uint8_t* pData,uint16_t N)
{

 HAL_I2C_Mem_Write(&hi2c2,HDMI_RX_I2C_SLAVE_ADDR,RegAddr,I2C_MEMADD_SIZE_8BIT,pData,N,10);
delay1ms(20);
}
...
USART1_printf( USART1,"2  %x   \r\n",0 );

	 ucRevision = HDMIRX_ReadI2C_Byte(REG_RX_PWD_CTRL1);
	#if DEBUG_MSG
...
USART1_printf( USART1,"4  %x   \r\n",0 );
    ...
    //AcceptCDRReset = TRUE; //leo del
USART1_printf( USART1,"5  %x   \r\n",0 );
    #ifndef MANUAL_TURNON_HDMIRX
    if(!bFullInit)
    {
...
IsSCDT(void)
{
    ...
    uc = HDMIRX_ReadI2C_Byte(REG_RX_SYS_STATE)&(B_SCDT|B_VCLK_DET);
    return(uc==(B_SCDT|B_VCLK_DET))?TRUE:FALSE ;
}
...
//	 USART1_printf( USART1,"CheckPlg5VPwr()  %x   \r\n",uc );
...
SetIntMask2(BYTE AndMask,BYTE OrMask)
{
	...
	i = HDMIRX_ReadI2C_Byte(REG_RX_INTERRUPT_MASK2);
	  HDMIRX_WriteI2C_Byte(REG_RX_INTERRUPT_MASK2, i);
}
...
IsHDMIRXHDMIMode(void)
{
   // BYTE uc ;
    ...
    //return (uc&B_HDMIRX_MODE)?TRUE:FALSE ;//leo
    return TRUE ;
}
...
Interrupt_Handler(void)
{
	...
	BYTE sys_state ;
	 #endif	
//	BYTE flag = FLAG_CLEAR_INT_ALL; //leo del
...
#endif	
 USART1_printf( USART1," sys_state %x   \r\n",HDMIRX_ReadI2C_Byte(REG_RX_SYS_STATE)); 
...
void
RXINT_5V_PwrOff(void)
{
    //BYTE sys_state ;

   // sys_state = HDMIRX_ReadI2C_Byte(REG_RX_SYS_STATE); //leo del
...
RXINT_AVMute_Set()
{
    ...
    SetIntMask2((~(B_PKT_CLR_MUTE))& 0xFF,(B_PKT_CLR_MUTE)); // enable the CLR MUTE interrupt.
...
RXINT_AVMute_Clear()
{
    ...
    SetIntMask2((~(B_PKT_CLR_MUTE))& 0xFF,0); // clear the CLR MUTE interrupt.
}
...
SwitchVideoState(Video_State_Type state)
{
    ...
    case VSTATE_VideoOn:
        ...
        else
        {
            ...
            SetIntMask2((~(B_NEW_AVI_PKG|B_PKT_SET_MUTE|B_PKT_CLR_MUTE))& 0xFF,(B_NEW_AVI_PKG|B_PKT_SET_MUTE|B_PKT_CLR_MUTE));
            SetIntMask1(~(B_SCDTOFF|B_PWR5VOFF),(B_SCDTOFF|B_PWR5VOFF));
            SetIntMask4(0,B_M_RXCKON_DET);
...
static void
SetColorimetryByMode(void)
{
    // USHORT HRes, VRes ;
    bInputVideoMode &= ~F_MODE_ITU601 ;
    ...
    else
    {
        ...
        bInputVideoMode &= ~F_MODE_ITU601 ; // set mode as ITU601
    }
}
...
SetColorimetryByInfoFrame()
{
...
#if (defined (SUPPORT_OUTPUTYUV))&& (defined (SUPPORT_INPUTRGB))
    uint8_t _CODE bCSCMtx_RGB2YUV_ITU601_16_235[] =
    {
    ...
    uint8_t _CODE bCSCMtx_RGB2YUV_ITU601_0_255[] =
    {
    ...
    uint8_t _CODE bCSCMtx_RGB2YUV_ITU709_16_235[] =
    {
    ...
    uint8_t _CODE bCSCMtx_RGB2YUV_ITU709_0_255[] =
    {
...
#if (defined (SUPPORT_OUTPUTRGB))&& (defined (SUPPORT_INPUTYUV))
    uint8_t _CODE bCSCMtx_YUV2RGB_ITU601_16_235[] =
    {
    ...
    uint8_t _CODE bCSCMtx_YUV2RGB_ITU601_0_255[] =
    {
    ...
    uint8_t _CODE bCSCMtx_YUV2RGB_ITU709_16_235[] =
    {
    ...
    uint8_t _CODE bCSCMtx_YUV2RGB_ITU709_0_255[] =
    {
...
uint8_t _CODE bCSCOffset_16_235[] =
{
    0x00,0x80,0x00
};

uint8_t _CODE bCSCOffset_0_255[] =
{
    0x10,0x80,0x10
};
