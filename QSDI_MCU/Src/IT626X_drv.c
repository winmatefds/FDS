...
static void HDCP_CancelRepeaterAuthenticate();
...
static void SetupAFE(BYTE level);
...
BYTE HDMITX_ReadI2C_Byte(BYTE RegAddr)
{
   ...
   // p_data=ReadI2C_Byte(HDMI_TX_I2C_SLAVE_ADDR, RegAddr);
 HAL_I2C_Mem_Read(&hi2c2, HDMI_TX_I2C_SLAVE_ADDR, RegAddr, 1, &p_data, 1, 1000);
delay1ms(20);

    return p_data;
}

void  HDMITX_WriteI2C_Byte(uint8_t RegAddr, uint8_t pData)
{

 HAL_I2C_Mem_Write(&hi2c2,HDMI_TX_I2C_SLAVE_ADDR,RegAddr,I2C_MEMADD_SIZE_8BIT,&pData,1,10);
 delay1ms(20);
}


void  HDMI_AndREG_TX_Byte(uint8_t RegAddr, uint8_t d)
{
   // uint8_t flag;

    HDMITX_WriteI2C_Byte(RegAddr,(HDMITX_ReadI2C_Byte(RegAddr) & (d)));

  
}

void  HDMI_OrREG_TX_Byte(uint8_t RegAddr, uint8_t d)
{
   // uint8_t flag;
...
void  HDMI_SetREG_TX_Byte(uint8_t RegAddr, uint8_t andmask,uint8_t ormask)
{
   // uint8_t flag;

   HDMITX_WriteI2C_Byte(RegAddr,((HDMITX_ReadI2C_Byte(RegAddr) & (andmask))|(ormask)));

  
}


void  HDMITX_AndREG_Byte(uint8_t RegAddr, uint8_t d)
{
   // uint8_t flag;
...
void  HDMITX_OrREG_Byte(uint8_t RegAddr, uint8_t d)
{
   // uint8_t flag;
...
void  HDMITX_SetREG_Byte(uint8_t RegAddr, uint8_t andmask,uint8_t ormask)
{
   // uint8_t flag;
...
BYTE LVDS_ReadI2C_Byte(uint8_t RegAddr)
{
    BYTE p_data;

//    p_data=ReadI2C_Byte(0x66, RegAddr);
 HAL_I2C_Mem_Read(&hi2c2, 0x66, RegAddr, 1, &p_data, 1, 1000);
delay1ms(20);
    return p_data;
...
void  LVDS_WriteI2C_Byte(uint8_t RegAddr, uint8_t d)
{

   HAL_I2C_Mem_Write(&hi2c2,0x66,RegAddr,I2C_MEMADD_SIZE_8BIT,&d,1,10);
delay1ms(20);
}
...
void  LVDS_AndREG_TX_Byte(uint8_t RegAddr, uint8_t d)
{
   // uint8_t flag;

    LVDS_WriteI2C_Byte(RegAddr,(LVDS_ReadI2C_Byte(RegAddr) & (d)));

  
}

void  LVDS_OrREG_TX_Byte(uint8_t RegAddr, uint8_t d)
{
   // uint8_t flag;
...
void  LVDS_SetREG_TX_Byte(uint8_t RegAddr, uint8_t andmask,uint8_t ormask)
{
   // uint8_t flag;

   LVDS_WriteI2C_Byte(RegAddr,((LVDS_ReadI2C_Byte(RegAddr) & (andmask))|(ormask)));
...
void  I2C_AndReg_Byte(uint8_t RegAddr, uint8_t d)
{
   // uint8_t flag;
...
void  I2C_OrReg_Byte(uint8_t RegAddr, uint8_t d)
{
   // uint8_t flag;
...
void  I2C_SetReg_Byte(uint8_t RegAddr, uint8_t andmask,uint8_t ormask)
{
   // uint8_t flag;
...
void  Switch_HDMITX_Bank( uint8_t d)
{
uint8_t val=(d)&1;

 HAL_I2C_Mem_Write(&hi2c2,HDMI_TX_I2C_SLAVE_ADDR,0x0f,I2C_MEMADD_SIZE_8BIT,&val,1,10);
...
//	  LVDS_OrREG_TX_Byte(0x2c,0x80);//SRAM RESET
#ifdef IT626X_SSC
...
BOOL ReGenTiming(BYTE fmt_assign)
{
    ...
    int VRS, VRE;
    int VDES, VDEE   ;
    int VRS2ndRise, VRS2nd, VRE2nd;
    int VDES2nd, VDEE2nd;
    int HTotal, VTotal;
    int i;
    int H_Total, H_FBH, H_SyncW, H_BBH, H_DEW;
    int V_Total,  V_FBH,  V_SyncW,  V_BBH, V_DEW;
    int SyncPol;
    BYTE fmt_index;
    BYTE interlaced;
...
//	   HDMITX_WriteI2C_Byte(0x72,0) ;//LEO
...
BYTE intdata2;
	...
	if((sysstat & (B_HPDETECT)) == (B_HPDETECT))
	{
    	HPD = TRUE;
    }
	else
    ...
    if(sysstat & B_INT_ACTIVE)
    {
        ...
        if(intdata1 & B_INT_AUD_OVERFLOW)
        {
          ...
          //  AudioDelayCnt=AudioOutDelayCnt; //leo ??????
            LastRefaudfreqnum=0;
        }
#if Debug_message
...
DisableVideoOutput(void)
{
    ...
    HDMITX_WriteI2C_Byte(REG_TX_AFE_DRV_CTRL,B_AFE_DRV_RST|B_AFE_DRV_PWD);
    Instance[0].bVideoOut=FALSE;
	USART1_printf( USART1," DisableVideoOutput",0); 
...
AutoAdjustAudio(void)
{
 ...
 //   else if (SampleFreq < 800000L ) {fs = AUDFS_768KHz ;}
    ...
    if(Instance[0].bAudFs != fs)
    {
        ...
        SetNCTS(Instance[0].bAudFs); // set N, CTS by new generated clock.
        //CurrCTS=0;
        return;
    }
    return;
...
//    BYTE revoked ;
    ...
    if( cDownStream > 6 || BStatus & (B_MAX_CASCADE_EXCEEDED|B_DOWNSTREAM_OVER))
    {
#if Debug_message
...
{
  //  BYTE AudioEnable;//, AudioFormat ; //leo
    BYTE i ;
...
EnableHDMIAudio(BYTE AudioType, BOOL bSPDIF,  ULONG SampleFreq,  BYTE ChNum, BYTE *pIEC60958ChStat, ULONG TMDSClock)
{
    ...
    BYTE Fs ;
	BYTE SUPPORT_AUDI_AudSWL=20;//leo add
   ...
   //     pIEC60958ChStat[4] |= (((~AUDFS_768KHz)<<4) & 0xF0)| 0xB ;
    ...
    case T_AUDIO_LPCM:
        ...
        setIT626X_ChStat(pIEC60958ChStat);
        setIT626X_LPCMAudio((ChNum+1)/2, SUPPORT_AUDI_AudSWL, bSPDIF);
        // can add auto adjust
        break ;
    }
   // HDMITX_AndREG_Byte(REG_TX_INT_MASK1,~(B_AUDIO_OVFLW_MASK));
   HDMITX_AndREG_Byte(REG_TX_INT_MASK1,0x8f);
    HDMITX_AndREG_Byte(REG_TX_SW_RST,~(B_AUD_RST|B_AREF_RST));
}
...
{
    BYTE uc,level;
    unsigned long cTMDSClock = PixelClock;
    uc = HDMITX_ReadI2C_Byte(0xc1);
    switch(uc & 0x70)
    {
    case 0x50:
...
void DumpLVDSReg()
{
    ...
    for(i = 0 ; i < 0x100 ; i+=16 )
    {
        ...
        for( j = 0 ; j < 16 ; j++ )
        {
            i2c_read_byte(0x66, (i+j), 1, &ucData, 0);
