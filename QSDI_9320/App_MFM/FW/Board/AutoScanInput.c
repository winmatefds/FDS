...
//   if(PwrState!=Pwr_On&& PwrState!=Pwr_Sleep )
      ...
      if (UserPrefPipMode != NO_PIP)
      {
         ...
         if ((AutoScanCondition(UserPrefCurrentInputPip) && !gm_TimingIsStable(gmvb_PipChannel)))
         {
			...
			SelectNextVideoInput(&UserPrefCurrentInputPip);
               ...
               if(((UserPrefCurrentInputMain == ALL_BOARDS_CVBS1) ||(UserPrefCurrentInputMain == ALL_BOARDS_SVIDEO1)) && ((UserPrefCurrentInputPip == ALL_BOARDS_CVBS1) ||(UserPrefCurrentInputPip == ALL_BOARDS_SVIDEO1)))
               	{
