...
#define IMAGE_SYNC_DP_MST_WAIT_TIME_OUT 400 // 4seconds
...
#if (NON_SCALING_OUTPUT == 1) && (DEINTERLACED_OUTPUT == 0)
...
static void ModeSetupEnd(gmt_PHY_CH B_Channel)
{
   ...
   if (gm_IsValidSignal(B_Channel))
   {
      ...
      //ChengLin 20130402: Remove to mode determine
      //if (B_Channel == gmvb_MainChannel)
...
#if 0 //(CHIP_ID != STDP93xx)   // Louis, Assume it's resolved in new IP
#ifdef DP_QD881_PATCH //KevinL 20090616
   if (gm_IsInputDP(CH_A))
   {
      //	BYTE B_DP_M_value;
      //B_DP_M_value = gm_ReadRegByte(DPRX_MS_M);
      //gm_Print("DPRX M : 0x%x",B_DP_M_value);

      if (((gm_ReadRegWord(IMP_CONTROL) & IMP_YUV_EN) && !gm_IsInterlaced(B_Channel)))
      {
         gm_WriteRegWord(DPRX_MS_CTRL, 0x03);
         gm_SetRegBitsByte(DPRX_SDP_CTRL, DPRX_SDP_VBID_M_DIS);
      }
      else
      {
         if (gm_ReadRegWord(DPRX_MS_CTRL) != 0x01)
            gm_WriteRegWord(DPRX_MS_CTRL, 0x01);

         if (gm_ReadRegByte(DPRX_SDP_CTRL) & DPRX_SDP_VBID_M_DIS)
            gm_ClearRegBitsByte(DPRX_SDP_CTRL, DPRX_SDP_VBID_M_DIS);
      }
   }
#endif
#endif
   ...
   if (gm_IsGraphics(B_Channel) && (AfeGetChannelPortProc(B_Channel) == AIP)
   ...
   //SWENG1200201 Modified for power on 1st time DpTX no display in Passthru mode 
#if (FEATURE_DISPLAY_PORT == ENABLE) && (NON_SCALING_OUTPUT == 1) 
   #if (PASS_THRU_MODE == 1)     
      ...
      DpTxEnableVideo(DPTX_TIMING_BY_INPUT);
		...
		if(GetChipID_4K2K()==CHIPID_4K2K_FE)
		{
			if ((B_Channel == CH_A))
			{
		   	//gm_Printf("~~~~ LVTX_IN_SEL_IMP_CLK ~~~~", 0);
		   	gm_ClkSelect(CLK_LVTX_IN, LVTX_IN_SEL_IMP_CLK);
         	gm_ClkSelect(CLK_ODP_CAPTURE, ODP_CAPTURE_SEL_IMP_CLK);
         	gm_ClkSelect(CLK_ODP, ODP_SEL_ODP_PLL_CLK); // ODP_CLK must before DCLK_SEL
				   
			   if(((stInput[B_Channel].Vtotal <= 4095) && (stInput[B_Channel].Htotal <= 4095) /*&& (stInput[B_Channel].Vactive >= 471)*/) || (gm_IsInterlaced(gmvb_MainChannel)))
			   {
				   DpTxEnableVideo(DPTX_TIMING_BY_OUTPUT);
   
				   //gm_Print("Scaling mode .... %d", 0);
   		      #ifdef ST_4K2K_93xx_BOARD
   		      #if (FEATURE_HDMI == ENABLE && ENABLE_VEGA_CONTROL_VIA_ATHENA_FE && ENABLE_VEGA_CSC_VIA_HDMI_INFO)
   		      if((GetChannelConnType(B_Channel) == HDMI) && (B_Channel == gmvb_MainChannel))
   		      {
   			      SOCIPC_VEGA_CSC_Via_HDMI_Input(FALSE); 	   
   		      }
   		      #endif
   		      #endif         
				}
				...
				{	
				   ...
				   ClearOdpTiming();
				   gm_ForceUpdate(B_Channel,OUTPUT_CLOCK);
				   DpTxEnableVideo(DPTX_TIMING_BY_INPUT);
				   gm_SetRegBitsWord(BYPASS,PASS_THROUGH);
			    
				   /*ChengLin 20130205: Program VEGA CSC Register via HDMI input status*/
				   #ifdef ST_4K2K_93xx_BOARD
				   #if (FEATURE_HDMI == ENABLE && ENABLE_VEGA_CONTROL_VIA_ATHENA_FE && ENABLE_VEGA_CSC_VIA_HDMI_INFO)
					if((GetChannelConnType(B_Channel) == HDMI) && (B_Channel == gmvb_MainChannel))
				   {
					   SOCIPC_VEGA_CSC_Via_HDMI_Input(TRUE);		
				   }
				   #endif	 
				   #endif		 
				   //gm_Print("PassTHRU mode .... %d", 0);
				}
			}
		}
	/*   else if(GetChipID_4K2K()==CHIPID_4K2K_R)
		...
		else if(GetChipID_4K2K()==CHIPID_4K2K_L)
		{
			if ((B_Channel == CH_A))
			{
						if(MemoryCheck_4Kx2K() == TRUE)//Modify by ChengLin 20130318
						{
							gm_WriteRegWord(IMP_H_ACT_WIDTH, stInput[gmvb_MainChannel].Hactive + 8);
							if(stInput[gmvb_MainChannel].Hactive > PanelWidth)
								gm_WriteRegWord(MFRC_PIXELS_OUT, PanelWidth);
							else
								gm_WriteRegWord(MFRC_PIXELS_OUT, stInput[gmvb_MainChannel].Hactive + 8); 
						}
						else
						{
							...
							if(stInput[gmvb_MainChannel].Hactive > PanelWidth)
								...
								gm_WriteRegWord(MFRC_PIXELS_OUT, stInput[gmvb_MainChannel].Hactive); 
						}
					}
		}
...
#else
//BYTE AthenaOutputTiming = T1280x720_60Hz;
void PanelSetAsInputTiming (void) //leo add
{
   ...
   switch(UserPrefOutputTiming)
   {
      case T1280x720_60Hz:
         gm_Print("xxxxxxxxxxxxxx T1280x720@60 xxxxxxxx",0);
   		PanelWidth = 1280;
   		PanelHeight = 720;
   		PanelMaxVFreq = 60;
   		PanelMinVFreq = 60;
   		...
   		Panel22HTotal = 1650;
   		Panel33HTotal = 1650;
   		PanelMinHSyncWidth = 40;
   		PanelMinHSyncBackPorch = 220;
   		...
   		PanelMinVSyncWidth  = 5;

   		PanelMinVSyncBackPorch =20;
   		gmvw_OutputHStart[gmvb_MainChannel] = 0;
   		gmvw_OutputVStart[gmvb_MainChannel] = 0;
   		gmvw_OutputHActive[gmvb_MainChannel] = 1280;
   		gmvw_OutputVActive[gmvb_MainChannel] = 720;
  // 		UserPrefIsDyanmicOutput4K2K_Flag=FALSE;
      break;
      case T1920x1080_30Hz:
         gm_Print("xxxxxxxxxxxxxx 1920x1080@30 xxxxxxxx",0);
   		PanelWidth = 1920;
   		PanelHeight = 1080;
   		PanelMaxVFreq = 30;
   		PanelMinVFreq = 29;
   		...
   		Panel22HTotal = 2200;
   		Panel33HTotal = 2200;
   		PanelMinHSyncWidth = 44;
   		PanelMinHSyncBackPorch = 148;
...

{
   ...
   gm_MemorySetup(B_Channel); // Video Memory
	...
	if(B_Channel == CH_A)
	{
		if(gm_IsInterlaced(CH_A))
			UserPrefImageRotation = OSD_NON_ROTATED;
   ...
   if (B_Channel == CH_A)
   {
      // Set OSD to center for non-scaling output
      gm_WriteRegWord(OSD_H_ACTIVE, (stInput[B_Channel].Hactive /2));
      gm_WriteRegWord(OSD_V_ACTIVE, (stInput[B_Channel].Vactive));
      gm_ForceUpdate(CH_A, INPUT_OUTPUT_CLOCK);

 #if (DEINTERLACED_OUTPUT == 0)
      BypassMemory();
 #endif
   }
...
//	gm_SyncUpdate(gmvb_MainChannel, INPUT_OUTPUT_CLOCK);
...
#if 0//(FEATURE_AIP == ENABLE) && (ENABLE_DFL == 0)
	if((B_Channel == CH_A) && (GetChannelConnType (B_Channel) == VGA) && (IsGraphics(B_Channel) == TRUE))
	{
   ...
   if(GetChipID_4K2K()!=CHIPID_4K2K_FE) 
   {
		...
		{
         /*ChengLin 20130722: To separate FULL/PBP mode main capture window will follow virtual payload number or not*/
         if(UserPrefCaptureM != CAPTURE_MODE_HALF_HORIZONTAL)  // Modify to control by UserPrefPBPmpde later
         {
         ...
         {
            //gm_Print("PBP Mode in DPRX0 or DPRX1", 0);
            gm_4K2K_MainCapture();
         }
		}
	}
...
//if  (UserPrefAUTOADJUST==1)
  if (GetChannelConnType(B_Channel) == VGA) // Only work for VGA port, not Component.
   {
      //make sure autoadjust is not performed when pip cannot be seen on the screen
      if (!((B_Channel == gmvb_PipChannel) && (UserPrefPipMode == NO_PIP)))
      {
       //  if (B_AutoAdjustCount[B_Channel] < MAX_AUTO_ADJUST || MAX_AUTO_ADJUST == 0)
         {
            ...
            OSDRequestMsg(B_Channel, MTO_AutoAdjustStart, DISPLAY_DEFER_TMO_NORMAL + 800);
            ModeSetupRequest(B_Channel, REQ_MS_AUTO_ADJUST, PARAM_NONE, DISPLAY_DEFER_TMO_NORMAL + 1500);
   ...
   */
   ...
   if(HdmiLoss(B_Channel)) // 7 //  ms
   {
      ...
      gm_ClkSelect(CLK_ODP_CAPTURE, ODP_CAPTURE_SEL_IMP_CLK);
		...
		//	PanelSetAsInputTiming(); // must before gm_MemorySetup 
		//	MemoryAllocate(gmvb_MainChannel); // For max capture memory upadate
		//	DisplaySetTiming();
...
void ModeSetupUserPref(gmt_PHY_CH B_Channel)
{
   ...
   if (B_Channel == CH_A)
   {
      ...
      if ((gm_IsVideo(CH_A) == TRUE) ||(gm_IsGraphics(CH_A) && gm_IsInterlaced(CH_A) ) )
      {
         ...
         //
         if ((UserPrefMADI == MADI_OFF) || (stInput[CH_A].Hactive > 1920) || (stInput[CH_A].Vactive > 1080))
...
void ModeSetupAutoAdjust(gmt_PHY_CH B_Channel)
{
   ...
   if (IsEstimatedMode(B_Channel))
      IaRetStatus = gm_AutoGeometry( B_Channel, gmd_FORCE_WIDTH);
   else
...
void SetDisplayOff(gmt_PHY_CH B_Channel, t_DISP_OFF_MODE B_Mode)
{
   ...
   if ((B_Mode == BACKGROUND_ONLY)||(B_Mode == BACKGROUND_ONLY_NO_SYNC_UPDATE))
      gm_DisplayForceBkgnd(B_Channel, TRUE, (B_Channel == gmvb_MainChannel)?
                           RGB_565(MAIN_BACKGROUND_COLOR):RGB_565(PIP_BACKGROUND_COLOR));
   else
...
//******************************************************************************
//
// FUNCTION       : void UpdateFilters(void)
//
...
void GameModeControl(void)
{
   BYTE B_GameModeCheck = UserPrefGameMode;
   
	//if((abs(stInput[gmvb_MainChannel].Vfrq - 600) > 10) || (!gm_IsGraphics(gmvb_MainChannel)))
      ...
      if (B_DisplayLockMethod == DISP_LOCK_NORMAL)
		{
         ...
         //gm_Printf("~~~~ Change Back to DFL ~~~~", 0);
         #else
         B_DisplayLockMethod = DISP_FREERUN;
         //gm_Printf("~~~~ Change Back to Free Run ~~~~", 0);         
         #endif
      }
		//gm_Print("V Freq. not 60Hz ... Disable Game Mode ...", 0);
	}

	if(B_GameModeCheck == 0)
	{
   ...
   #if 0 // These should be set via modedetermine		
   // Update Register
   if (gm_ReadRegWord(IMP_CONTROL) & IMP_EXT_DV_EN)
   {
      gm_WriteRegWord(IMP_H_ACT_START, 0);
	...
	if(GetChipID_4K2K()==CHIPID_4K2K_L)
	{
      ...
      else
      {
         ...
         if(((UserPrefOutputAspectRatio == OUTPUT_AR_4_3) && (UserPrefCurrentInputMain == ALL_BOARDS_DP1)) || /*&& (UserPrefDPSWitch == DP_Input_11)*/
            ...
            result = FALSE;
         else if(UserPrefCurrentInputMain == ALL_BOARDS_DP2) //HDMI Path /*ChengLin 20130710: HDMI Path always "+8" for meet MFC calculation*/
            result = TRUE;             
         else
         {
...
//******************************************************************************
// FUNCTION:   CheckSplashScreenAndBlockHDMISyncDetection
// USAGE: Check Splash Screen is showing or not, once splash screen is showing, system will stop HDMI signal detection
//
// INPUT:  None
// OUTPUT: TURE or FALSE
// GLOBALS:
//******************************************************************************
BOOL CheckSplashScreenAndBlockHDMISyncDetection(void)
{
#ifdef ST_4K2K_93xx_BOARD
   if(GetChipID_4K2K() == CHIPID_4K2K_FE)
   {
      if (B_ModeSetupStarted == FALSE && B_SplashComplete == FALSE)
         return TRUE;
      else
         return FALSE;
   }
