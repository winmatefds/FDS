...
source = gmd_AUDIO_SRC_AUTO;


//return gmd_AUDIO_IN_LINE_IN;
 // if((UserPrefCurrentInputMain == ALL_BOARDS_VGA1)|| (UserPrefCurrentInputMain == ALL_BOARDS_DVI1))
		//	source= gmd_AUDIO_IN_LINE_IN;
...
return gmd_AUDIO_IN_LINE_IN;
   	...
   	return gmd_AUDIO_IN_I2S_IN;
  */
   switch (source)
   {
   default:
   case gmd_AUDIO_SRC_AUTO:
...
gmt_AUDIO_OUTPORT_TYPE AAPI_Audio_Get_CurrentOutput(gmt_PHY_CH B_Channel)
{
 ...
 B_Channel = gmvb_MainChannel;
   ...
   return gmd_AUDIO_OUT_LINE_OUT;//LEO
...
void AAPI_Audio_Cus_Initialize(void)
{
   ...
   SYS_API_AUDIO_Set_DAC_GAIN_Fixed_Value(0);   // fixed internal DAC gain. 0: normal, others: target gain
   SYS_API_AUDIO_Set_Analog_Fixed_Volume(0);    // fixed volume for analog input. 0: normal, others: target volume
...
gmt_AUDIO_INPORT_TYPE Audio_AutoScan_Get_NextPort(void)
{
   ...
   if ((GetUserPrefAudioPipInput() == gmd_AUDIO_SRC_AUTO)
...
if(UserPrefAudioMainInput==gmd_AUDIO_SRC_AUTO)
{
	if(UserPrefCurrentInputMain==ALL_BOARDS_HDMI)
		AUDIO_IS_AS_INDEX=gmd_AUDIO_IN_HDMI_I2S;
	else if(UserPrefCurrentInputMain==ALL_BOARDS_DP1)
		AUDIO_IS_AS_INDEX=gmd_AUDIO_IN_DPRx1_I2S;
	else
		AUDIO_IS_AS_INDEX=gmd_AUDIO_IN_LINE_IN;
}
else
	AUDIO_IS_AS_INDEX=gmd_AUDIO_IN_LINE_IN;

   //   AUDIO_IS_AS_INDEX ++; //leo del
   }
}
...
void Audio_Initialize(void)
{
   ...
   case BEHAVIOR_ID_93xx_ST_4K2K_FE:
      UserPrefAudioMainInput  = gmd_AUDIO_SRC_AUTO;
...
UserPrefAudioMainInput  = gmd_AUDIO_SRC_AUTO;
UserPrefAudioMainOutput = gmd_AUDIO_TAR_LINE_OUT ;
