...
static void PCA6416A_ByteWrite(BYTE B_Address, BYTE B_Data)
{
   BYTE Ba_Buffer[2];
   ...
   Ba_Buffer[1] = B_Data;
	...
	if (gm_Write2WireBlock(1, 0x40, (BYTE*)Ba_Buffer, 2, TRUE) == gmd_PASS)
    {
     //    gm_Printf("Write command   PCA6416A by I2C 0x42", 0);
    }
    else
    {
      gm_Printf("Write command  PCA6416A Failed=%x",B_Address);
    }
...
UserPrefLED_Setting=1;
PCA6416A_ByteWrite(0x02,0x00);
   if((gm_ReadRegWord(GPINPUT4) & GPIO79_IN) && (gm_ReadRegWord(GPINPUT5) & GPIO80_IN))
		UserPrefOutputTiming = T1920x1080_60Hz;
    else  if((gm_ReadRegWord(GPINPUT4) & GPIO79_IN) || (gm_ReadRegWord(GPINPUT5) & GPIO80_IN))
		UserPrefOutputTiming = T1920x1080_30Hz;
    else
		UserPrefOutputTiming = T1280x720_60Hz;
...
void far gm_MainLoop (void)
{
 ...
 //  BYTE B_Data;
  // B_Data=0x42;
 //  gm_Write2WireBlock(0, 0x42, (BYTE *)&B_Data, 1, FALSE);
  
   
//gm_Printf("adv7180 =%x",gm_Read2WireBlock(0, 0x42, (BYTE*) &B_Data, 1, TRUE));
#if 0 // For test only
...
PCA6416A_ByteWrite(0x06,0x00);
PCA6416A_ByteWrite(0x07,0xf1);
#if 1

if(UserPrefLED_Setting==1)
{
         if (UserPrefCurrentInputMain==ALL_BOARDS_VGA1)
		PCA6416A_ByteWrite(0x02,0x04);//VGA
	  else if (UserPrefCurrentInputMain==ALL_BOARDS_CVBS1)
		PCA6416A_ByteWrite(0x02,0x08);//CVBS
	  else if (UserPrefCurrentInputMain==ALL_BOARDS_DP1)
	  	PCA6416A_ByteWrite(0x02,0x10);//DP
	  else if (UserPrefCurrentInputMain==ALL_BOARDS_HDMI)
	  	PCA6416A_ByteWrite(0x02,0x01);//DVI
	   else if (UserPrefCurrentInputMain==ALL_BOARDS_SVIDEO1)
	   	PCA6416A_ByteWrite(0x02,0x02);//SDI
	...
	PCA6416A_ByteWrite(0x02,0x00);
         ...
         if (PanelWidth==1920 && PanelHeight == 1080 && PanelMaxPClk >= 140000L )
         	{
         	 if(UserPrefCurrentInputMain !=ALL_BOARDS_SVIDEO1)
			PCA6416A_ByteWrite(0x03,0x04);
		else
			PCA6416A_ByteWrite(0x03,0x02);

         	}
	  else 
	  	{
	  	 if(UserPrefCurrentInputMain !=ALL_BOARDS_SVIDEO1)
	  		PCA6416A_ByteWrite(0x03,0x00);
		else
			PCA6416A_ByteWrite(0x03,0x02);
