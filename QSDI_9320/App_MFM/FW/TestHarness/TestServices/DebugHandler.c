...
#ifdef MONITOR_MAIN_LOOP_TIME
  BYTE MainLoopTimeRec[50];
#endif

#ifdef INDIVIDUAL_HANDLER_TIME
WORD W_HandlerPrevTime[MAX_DBG_HDLR];
WORD W_HandlerMaxTime[MAX_DBG_HDLR];
#endif
...
static WORD W_PrevTime = 0xFFFF, W_CurrentTime;		// System time
...
void DebugHandler(void)
{
   ...
   else
   {
      ...
      {
         B_MainLoopLimit = TRUE;
         W_MaxTime = W_CurrentTime;
      }
   }
 #endif //MONITOR_MAIN_LOOP_TIME
   ...
   if (gm_TimerCheck(DEBUG_100ms_TMR) == TIMER_TMO)
   {
      ...
      {
         gm_Print("Max Main Loop Time: %d ms",W_MaxTime);
         B_MainLoopLimit = FALSE;
         W_MaxTime = 0;
      ...
      if (Bp_Addr <= W_StackEnd)
         gm_Print("!!! Stack Overflow !!!",0);
      else if (Bp_Addr < W_StackMark)     // Stack move
      {
         W_StackMark = Bp_Addr;
         gm_Print("Current Max Used Stack Size = %d",W_StackStart - W_StackMark);
      }
    #endif
   }
   //
   ...
   {
      static BYTE pre_Main = 0xFF, pre_Pip = 0xFF;
      if ((pre_Main != UserPrefCurrentInputMain) || (pre_Pip != UserPrefCurrentInputPip))
      {
         gm_Print("--->UserPrefCurrentInputMain=%d",UserPrefCurrentInputMain);
         gm_Print("--->UserPrefCurrentInputPip=%d",UserPrefCurrentInputPip);
         pre_Main = UserPrefCurrentInputMain;
