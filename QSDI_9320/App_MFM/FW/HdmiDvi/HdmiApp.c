...
#else
#define Default_Hpd_Polarity_Reversed        0
#endif
...
BOOL gm_HdmiIsStableState(BYTE InputConnector)
{
   gmt_HdmiDviInputPort* ptr_PortConfig=&HdmiDviInPortConfig[GetInPortConfigArrayLocation(InputConnector)];
      ...
      if (PC_SignalDataLoaded == TRUE)
      {
         ...
         /*ChengLin 20130627: Check Splash Screen is showing or not, once splash screen is showing, system will stop HDMI signal detection*/
         if(CheckSplashScreenAndBlockHDMISyncDetection() == TRUE)
            return FALSE;
         else
            return TRUE;
         }
