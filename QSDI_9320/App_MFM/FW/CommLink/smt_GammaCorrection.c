   ...
   *Bp_Len = 1; //output message length
   ...
   switch (B_Command)
   {
      ...
      case Gamma_SaveProfile:
    		...
    		#endif
         {
            ...
            if(Bp_Reply [0] == Gamma_OK)
            {
               ...
               gm_MemoryCodeStart();               
                  //gm_Printf("3 gm_MemoryCodeStart %d", gm_GetSystemTime());                                    
               }
               #endif
            }
            #else // SAVING_DATA_INTO_FLASH
      ...
      case Gamma_EraseProfile:
		...
		#if (GAMMA_SMT_PROTOCOL_VER >= VERSION_6)
		case Gamma_HideColor:
			#ifdef IC2_DEBUG_PRINT
			gm_Print("Gamma_HideColor",0);
			#endif
            
			if (GetSMTICToolVersion() < VERSION_6)
				Bp_Reply[0] = Gamma_HideColor_Failure;
			else
			{
				if(GetSMTICToolVersion() >= VERSION_6 && GetSMTICStarted() == FALSE)
					ErrorCode = PORT_MSG_HNDLR_ERROR;
				...
				{            
					ProgramOVP3x3MatrixReg(W_OVP3x3MatrixSaved);
					gm_SetRegBitsWord(OVP_UPDATE_CTRL, OVP_SYNC_UPDATE); // debugon can't use gm_SyncUpdate
					Bp_Reply[0] = Gamma_HideColor_OK;
               #ifdef IC2_DEBUG_PRINT
               gm_Print("Gamma_HideColor_OK",0);
               #endif
				}
			} // if (GetSMTICToolVersion() < VERSION_6)
...
static void SetSMTICToolVersion(BYTE VersionNumber)
{
	B_SMTICVersion = VersionNumber;
	#ifdef IC2_DEBUG_PRINT
	gm_Print("IC PC Tool Version : %d", B_SMTICVersion);
	#endif
}

static BYTE GetSMTICToolVersion(void)
{
	return B_SMTICVersion;
}

static void SetSMTICStarted(BOOL Status)
{
	B_IsSMTICStarted = Status;
	#ifdef IC2_DEBUG_PRINT
	if (Status == TRUE)
		gm_Print("IC is started", 0);
	else
		gm_Print("IC is ended",0);
	#endif
}

static BOOL GetSMTICStarted(void)
{
	return B_IsSMTICStarted;
}
...
static PortMsgHndlrReplyType BackupOVP3x3Matrix(WORD * OVP3x3MatrixData)
{
	...
	if(OVP3x3MatrixData == NULL)
		ErrorCode = PORT_MSG_HNDLR_INVALID_PARAM;
	else
	{
		BYTE i = 0;

		//store 3x3 coef and offset
		//registers 3x3 coef, 12-bit (1.2.9)
		OVP3x3MatrixData[i++] = gm_ReadRegWord(MAIN_P_MULT_COEF11) & 0x0FFF;
		OVP3x3MatrixData[i++] = gm_ReadRegWord(MAIN_P_MULT_COEF12) & 0x0FFF;
		OVP3x3MatrixData[i++] = gm_ReadRegWord(MAIN_P_MULT_COEF13) & 0x0FFF;
		OVP3x3MatrixData[i++] = gm_ReadRegWord(MAIN_P_MULT_COEF21) & 0x0FFF;
		OVP3x3MatrixData[i++] = gm_ReadRegWord(MAIN_P_MULT_COEF22) & 0x0FFF;
		OVP3x3MatrixData[i++] = gm_ReadRegWord(MAIN_P_MULT_COEF23) & 0x0FFF;
		OVP3x3MatrixData[i++] = gm_ReadRegWord(MAIN_P_MULT_COEF31) & 0x0FFF;
		OVP3x3MatrixData[i++] = gm_ReadRegWord(MAIN_P_MULT_COEF32) & 0x0FFF;
		OVP3x3MatrixData[i++] = gm_ReadRegWord(MAIN_P_MULT_COEF33) & 0x0FFF;

		//output offset registers, 15-bits
		OVP3x3MatrixData[i++] = gm_ReadRegWord(MAIN_P_OUT_OFFSET1) & 0x7FFF;
		OVP3x3MatrixData[i++] = gm_ReadRegWord(MAIN_P_OUT_OFFSET2) & 0x7FFF;
		OVP3x3MatrixData[i++] = gm_ReadRegWord(MAIN_P_OUT_OFFSET3) & 0x7FFF;
	} // if(OVP3x3MatrixData == NULL)
...
static PortMsgHndlrReplyType ProgramOVP3x3MatrixReg(WORD * OVP3x3MatrixData)
{
	...
	else
	{
      ...
      #ifdef IC2_DEBUG_PRINT
      gm_Print("data to program 3x3",0);
      gm_Print("0x%x 0x%x 0x%x", OVP3x3MatrixData[0],OVP3x3MatrixData[1],OVP3x3MatrixData[2]);
      gm_Print("0x%x 0x%x 0x%x", OVP3x3MatrixData[3],OVP3x3MatrixData[4],OVP3x3MatrixData[5]); 
      gm_Print("0x%x 0x%x 0x%x", OVP3x3MatrixData[6],OVP3x3MatrixData[7],OVP3x3MatrixData[8]); 
      gm_Print("0x%x 0x%x 0x%x", OVP3x3MatrixData[9],OVP3x3MatrixData[10],OVP3x3MatrixData[11]);        
      #endif

		//restore 3x3 coef and offset
		//registers 3x3 coef, 12-bit (1.2.9)
		gm_WriteRegWord(MAIN_P_MULT_COEF11, OVP3x3MatrixData[i++] & 0x0FFF);
		gm_WriteRegWord(MAIN_P_MULT_COEF12, OVP3x3MatrixData[i++] & 0x0FFF);
		gm_WriteRegWord(MAIN_P_MULT_COEF13, OVP3x3MatrixData[i++] & 0x0FFF);
		gm_WriteRegWord(MAIN_P_MULT_COEF21, OVP3x3MatrixData[i++] & 0x0FFF);
		gm_WriteRegWord(MAIN_P_MULT_COEF22, OVP3x3MatrixData[i++] & 0x0FFF);
		gm_WriteRegWord(MAIN_P_MULT_COEF23, OVP3x3MatrixData[i++] & 0x0FFF);
		gm_WriteRegWord(MAIN_P_MULT_COEF31, OVP3x3MatrixData[i++] & 0x0FFF);
		gm_WriteRegWord(MAIN_P_MULT_COEF32, OVP3x3MatrixData[i++] & 0x0FFF);
		gm_WriteRegWord(MAIN_P_MULT_COEF33, OVP3x3MatrixData[i++] & 0x0FFF);

		//output offset registers, 15-bits
		gm_WriteRegWord(MAIN_P_OUT_OFFSET1, OVP3x3MatrixData[i++] & 0x7FFF);
		gm_WriteRegWord(MAIN_P_OUT_OFFSET2, OVP3x3MatrixData[i++] & 0x7FFF);
		gm_WriteRegWord(MAIN_P_OUT_OFFSET3, OVP3x3MatrixData[i++] & 0x7FFF);
	} // if(OVP3x3MatrixData == NULL)

	return ErrorCode;
}
#endif  //#if (GAMMA_SMT_PROTOCOL_VER >= VERSION_6)
