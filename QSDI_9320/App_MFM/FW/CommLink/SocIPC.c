...
#if ENABLE_PRINT_VEGA_FW_VERSION_FROM_ATHENA_R
...
/*ChengLin 20130222: Add for sync Left/Right PB Control Status*/
   ...
   FALSE, //SplashLoadDefault
	0, //GammaCorrection
   0, // AthenaFE_L_DebugMode    
	0, //VEGA_FW_4K2K_Version
	0 //VEGA_FW_Original_Version
};
...
//refer to doc "4Kx2K System FW SMT Design Gamma Calibration and Uniformity Compensation"
   ...
   case SICMD_RO_AthenaL_SyncStatus: //Added by ChengLin 20130621
      ((AthenaLSyncStatusResPkt_t fdata*)gExtDebugCmdPtr)->AthenaLSyncStatus = IsSyncOn();
      SendDataType = SocIPCGetResponseDataType(*(gExtDebugCmdPtr+1));   
      break;
#endif
...
#if ENABLE_PRINT_VEGA_FW_VERSION_FROM_ATHENA_R
   /*ChengLin 20130705: Get VEGA 4K2K FW Version Number*/
	case SICMD_RO_GetVEGA_4K2K_FW_Version:
		{
         WORD wCurrValue = 0;
         BYTE cFWChar = 0;
         BYTE cFW_Ver = 0;
         BYTE cFW_SubReversion = 0;
         BYTE cFW_Major_Minor_Rev = 0;         
         
         if(GetChipID_4K2K() == CHIPID_4K2K_FE)
         {
            //New VEGA I2C Address = 0x92 for read current 4K2K FW ID (After VEGA T05 firmware will support this address)
            wCurrValue = SOCIPC_VEGA_I2C_ReadWord_Via_FE(0x92); //VEGA T05 FW Only
            cFWChar = (BYTE)((wCurrValue&0xFF00)>>8);
            cFW_Ver = (BYTE)(wCurrValue&0x00FF);

			   ((GetVEGA4K2KFWResPkt_t fdata*)gExtDebugCmdPtr)->VEGA_FW_CHAR = cFWChar;
			   ((GetVEGA4K2KFWResPkt_t fdata*)gExtDebugCmdPtr)->VEGA_FW_NUM = cFW_Ver;

            //VEGA I2C Address = 0x04 for read original VEGA firmware version number            
            wCurrValue = SOCIPC_VEGA_I2C_ReadWord_Via_FE(0x04); //Firmware Version Nimber
            cFW_Major_Minor_Rev = (BYTE)((wCurrValue&0xFF00)>>8);
            cFW_SubReversion = (BYTE)(wCurrValue&0x00FF);

            ((GetVEGA4K2KFWResPkt_t fdata*)gExtDebugCmdPtr)->VEGA_FW_MAJOR_MINOR_REV = cFW_Major_Minor_Rev;
            ((GetVEGA4K2KFWResPkt_t fdata*)gExtDebugCmdPtr)->VEGA_FW_SUB_REV = cFW_SubReversion;   

         }
         else
         {
			   ((GetVEGA4K2KFWResPkt_t fdata*)gExtDebugCmdPtr)->VEGA_FW_CHAR = 0;
			   ((GetVEGA4K2KFWResPkt_t fdata*)gExtDebugCmdPtr)->VEGA_FW_NUM = 0;
            ((GetVEGA4K2KFWResPkt_t fdata*)gExtDebugCmdPtr)->VEGA_FW_MAJOR_MINOR_REV = 0;
            ((GetVEGA4K2KFWResPkt_t fdata*)gExtDebugCmdPtr)->VEGA_FW_SUB_REV = 0;            
         }
		 	SendDataType = SocIPCGetResponseDataType(*(gExtDebugCmdPtr+1));			
		}
		break;
	...
	else if( SendDataType == RECV_DATA )
	{
      ...
      #ifdef DP_HUB_2_LOG_PORTS   
        case SICMD_RO_AthenaL_SyncStatus: //Added by ChengLin 20130621
           ((AthenaLSyncStatusResPkt_t fdata*)gExtDebugCmdPtr)->Length = (sizeof(AthenaLSyncStatusResPkt_t)-2)+3;//+3:length,command,null-terminated
           SocIPCI2CSlaveWriteCmdPkt((BYTE far*)gExtDebugCmdPtr , B_whichHandler-I2C_SLAVE1_USE); 
         break;      
      #endif
         ...
         /*ChengLin 20130705: Get VEGA 4K2K FW Version Number*/    
	      case SICMD_RO_GetVEGA_4K2K_FW_Version:
				((GetVEGA4K2KFWResPkt_t fdata*)gExtDebugCmdPtr)->Length = (sizeof(GetVEGA4K2KFWResPkt_t)-2)+3;//+3:length,command,null-terminated
				SocIPCI2CSlaveWriteCmdPkt((BYTE far*)gExtDebugCmdPtr , B_whichHandler-I2C_SLAVE1_USE);
            break;
#endif            
			default: //ACK
...
/*********************************************/
      ...
      /*ChengLin 20130705: Get VEGA 4K2K FW Version Number*/
      case SICMD_RO_GetVEGA_4K2K_FW_Version:
			gAthena_LEFT_FE_Slave_Parameters.VEGA_FW_4K2K_Version = (WORD)(((((GetVEGA4K2KFWResPkt_t fdata*)gParabufferPtr)->VEGA_FW_CHAR)<<8)|
									(((GetVEGA4K2KFWResPkt_t fdata*)gParabufferPtr)->VEGA_FW_NUM));
			gAthena_LEFT_FE_Slave_Parameters.VEGA_FW_Original_Version = (WORD)(((((GetVEGA4K2KFWResPkt_t fdata*)gParabufferPtr)->VEGA_FW_MAJOR_MINOR_REV)<<8)|
									(((GetVEGA4K2KFWResPkt_t fdata*)gParabufferPtr)->VEGA_FW_SUB_REV));

			socipc_msg("\ngAthena_LEFT_FE_VEGA_FW_4K2K_Version = 0x%X.\n",gAthena_LEFT_FE_Slave_Parameters.VEGA_FW_4K2K_Version);
			socipc_msg("\ngAthena_LEFT_FE_VEGA_FW_Original_Version = 0x%X.\n",gAthena_LEFT_FE_Slave_Parameters.VEGA_FW_Original_Version);         
        break;
#endif
      ...
      case SICMD_RO_DpRx0_IsStable:
         ...
         break;
      case SICMD_RO_AthenaL_SyncStatus: //Added by ChengLin 20130621
         gAthena_LEFT_FE_Slave_Parameters.AthenaLSyncStatus = (((AthenaLSyncStatusResPkt_t fdata*)gParabufferPtr)->AthenaLSyncStatus);   
        break;           
#endif         
...
#if ENABLE_PRINT_VEGA_FW_VERSION_FROM_ATHENA_R //Add by ChengLin 20130708	
         case SIAPPTESTCMD_SHOW_CURR_VEGA_FW_REV:
            SOCIPC_VEGA_PrintFW_VersionViaAthenaFE();
         break;
#endif
         ...
         if((wCurrValue & 0x0008)== 0) //Re-Setup once VEGA cuurent mode is scaling mode
         {
            ...
            wCurrValue &= 0xFFE0;
			   ...
			   switch(ColorSpace)
			   {
				   ...
				   default:
                  if(gm_HdmiGetRGBQuanRange(gB_CurrentInputConnector[gmvb_MainChannel]) == gmd_RGB_QUANTIZATION_LIMITED)
                  {
                     wCurrValue|=0x0010; //CEA Range for VEGA
                     VEGA_ctrl_via_FE_msg("\n<VEGA DYNAMIC RANGE> LIMIT RANGE.\n",0);
                  }
                  else
                  {
                     VEGA_ctrl_via_FE_msg("\n<VEGA DYNAMIC RANGE> FULL RANGE.\n",0);
                  }
					   VEGA_ctrl_via_FE_msg("\n<VEGA COLOR FORMAT> RGB.\n",0);
					   break;
				   case STHDMIRX_COLOR_FORMAT_YUV_422:
					   VEGA_ctrl_via_FE_msg("\n<VEGA COLOR FORMAT> YUV422.\n",0);                  
                  switch (gm_HdmiGetYCCQuanRange(gB_CurrentInputConnector[gmvb_MainChannel]))
                  {
                     case gmd_YCC_QUANTIZATION_LIMITED:
                     case gmd_YCC_QUANTIZATION_UNKNOW:                        
                        wCurrValue|=0x0011; //CEA Range for VEGA
                        VEGA_ctrl_via_FE_msg("\n<VEGA DYNAMIC RANGE> LIMIT RANGE.\n",0);                       
                        break;
                     case gmd_YCC_QUANTIZATION_FULL:
					         wCurrValue|=0x0001;	                        
                        VEGA_ctrl_via_FE_msg("\n<VEGA DYNAMIC RANGE> FULL RANGE.\n",0);
                        break;
                  }
                  ...
                  switch (gm_HdmiGetYCCQuanRange(gB_CurrentInputConnector[gmvb_MainChannel]))
                  {
                     case gmd_YCC_QUANTIZATION_LIMITED:
                     case gmd_YCC_QUANTIZATION_UNKNOW:                        
                        wCurrValue|=0x0012; //CEA Range for VEGA
                        VEGA_ctrl_via_FE_msg("\n<VEGA DYNAMIC RANGE> LIMIT RANGE.\n",0);                       
                        break;
                     case gmd_YCC_QUANTIZATION_FULL:
					         wCurrValue|=0x0002;	                        
...
#if ENABLE_PRINT_VEGA_FW_VERSION_FROM_ATHENA_R
//******************************************************************************
// FUNCTION:   SOCIPC_VEGA_PrintFW_VersionViaAthenaFE
// USAGE:  Print VEGA 4K2K FW Version
//
// INPUT:  None
// OUTPUT:	TURE or FALSE
// GLOBALS:
//******************************************************************************
BOOL SOCIPC_VEGA_PrintFW_VersionViaAthenaFE(void)
{
   BOOL RetValue = FALSE;
	WORD wCurrValue = 0;
   BYTE cFW_Major_Minor_Rev = 0;
   BYTE cFW_SubVersion = 0;   
   BYTE cFWChar = 0;
   BYTE cFW_Ver = 0;
   char VEGA_FW_Original_Version[20];    
   char VEGA_FW_Version[20];
   char cFWCharBuff[3]={'\0'};

	if(GetChipID_4K2K() == CHIPID_4K2K_R)
   {  
      if(SocIPCSendCmdPacketParser(SOCIPC_DEVICEFE_DST_ADDRESS, SICMD_RO_GetVEGA_4K2K_FW_Version, NULL))
      {
         wCurrValue = gAthena_LEFT_FE_Slave_Parameters.VEGA_FW_Original_Version;
         cFW_Major_Minor_Rev = (BYTE)((wCurrValue&0xFF00)>>8);
         cFW_SubVersion = (BYTE)(wCurrValue&0x00FF);
         
         //gm_Print("\ncFW_Major_Minor_Rev = 0x%x.\n",cFW_Major_Minor_Rev);
         //gm_Print("\ncFW_SubVersion = 0x%x.\n",cFW_SubVersion); 
         
         _fstrcpy(VEGA_FW_Original_Version, (char *)"VEGA IROM Tx ?.?.?"); 
         AddToString((BYTE*)VEGA_FW_Original_Version,(BYTE)((cFW_Major_Minor_Rev&0xE0)>>5));
         AddToString((BYTE*)VEGA_FW_Original_Version,(BYTE)(cFW_Major_Minor_Rev&0x1F));
         AddToString((BYTE*)VEGA_FW_Original_Version,cFW_SubVersion);
         gm_Print((char *)VEGA_FW_Original_Version,0);   


         wCurrValue = gAthena_LEFT_FE_Slave_Parameters.VEGA_FW_4K2K_Version;
         cFWChar = (BYTE)((wCurrValue&0xFF00)>>8);
         cFW_Ver = (BYTE)(wCurrValue&0x00FF);
         ...
         _fstrcpy(VEGA_FW_Version, (char *)"4K2K VEGA FW Ver - "); 
         
         //Process 4K2K VEGA FW Character
         cFWCharBuff[0] = cFWChar;
         cFWCharBuff[1] = '?';      
         _fstrcat(VEGA_FW_Version, (char *)cFWCharBuff);
         
         AddToString((BYTE*)VEGA_FW_Version,cFW_Ver);
         
         gm_Print((char *)VEGA_FW_Version,0);    
