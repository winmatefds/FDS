...
void DynamicScalerStop(void)
{
   BOOL Bt_UpdateFilter[] = {FALSE, FALSE};
   {
      ...
      {
         if (!Bt_UpdateFilter[CH_A]) gm_ScalerDynamicWait(NUM_VSYNCS); // if absent it causes flash on screen
   ...
   {
      UpdateFilters();//update filters immediately so that there is no observable transient effect
   ...
   // For under 1/4 noise issue patch
   if(GetChipID_4K2K() != CHIPID_4K2K_FE)
   {
      if(gm_IsInterlaced(gmvb_MainChannel))
         gm_ClearRegBitsWord(MVF_CTRL, AFM_VT_MADI_SEL);
   }   
   #endif
}
/***********************************  END  ************************************/
