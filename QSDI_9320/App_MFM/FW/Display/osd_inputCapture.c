...
{
   WORD W_Limit = 0;
   if (gm_IsValidSignal(B_FocusBorder) == TRUE)
   {
    #if 1
      W_Limit = gmvw_InputHTotalOrg[B_FocusBorder] - 60;
...
void AdjustHSyncPhase(void)
{
   ...
   gmvb_InputSamplingPhase[B_FocusBorder] = UserPrefHSyncPhase;
   gm_SetAdcPhase(B_FocusBorder, UserPrefHSyncPhase);   //the 2 zeros are arbitrary
}
#endif
...
{
   gmvsw_InputHSyncOffset[B_FocusBorder] = UserPrefHSyncDelay;
...
{
   if (((SWORD)stInput[B_FocusBorder].Hstart - 8) > 100)
      return -100;
   else
      return (8 - (SWORD)stInput[B_FocusBorder].Hstart);
}
#endif //#ifdef ADJUSTER_HSTART_DECLARED
...
SWORD MaxHStart(void)
{
   if (((SWORD)stInput[B_FocusBorder].Hstart - 8) > 100)
      ...
      return ((SWORD)stInput[B_FocusBorder].Hstart - 8);
}
#endif //#ifdef ADJUSTER_HSTART_DECLARED
...
{
   gmvsw_InputVSyncOffset[B_FocusBorder] = UserPrefVSyncDelay;
...
{
   if (((SWORD)stInput[B_FocusBorder].Vstart - 4) > 100)
      return -100;
   else
      return (4 - (SWORD)stInput[B_FocusBorder].Vstart);
}
#endif //#ifdef ADJUSTER_VSTART_DECLARED
...
SWORD MaxVStart(void)
{
   if (((SWORD)stInput[B_FocusBorder].Vstart - 4) > 100)
      ...
      return ((SWORD)stInput[B_FocusBorder].Vstart - 4);
}
#endif //#ifdef ADJUSTER_VSTART_DECLARED
...
void OutputAspectOverride(void)
{
   ...
   /*ChengLin 20130619: Patch for Aspect 1:1 Athena L image fail issues*/
   if(GetChipID_4K2K()==CHIPID_4K2K_L)
   {
