...
void SwapMainPip(void)
{
         ...
         && (BlenderIsPending(CH_A) == FALSE))
   {
      ...
      if (B_SwapComplete == FALSE)
      {
         ...
         UserPrefCurrentInputPip = UserPrefPreviousInputMain;
...
#endif
	#if Two_Pass_Flip
	if(gm_IsInterlaced(CH_A) && (UserPrefImageRotation != OSD_NON_ROTATED))
	{
		gm_Print("Interlace and Two pass", 0);
		return;
	}
	#endif
...
//		gm_BlenderSetPriority((gmvb_MainChannel == CH_A)?PIP_ON_MAIN:MAIN_ON_PIP);
      ...
      B_PrevPipMode = UserPrefPipMode;
		...
		//if((UserPrefImageRotation != Off) && (UserPrefPipMode != NO_PIP))
      	//{
      	//	UserPrefImageRotation = Off;
      	//	AdjustImageRotation();
      	//}
