...
gm_Printf("UserPrefCurrentInputMain =%d",UserPrefCurrentInputMain);

#if 0//ndef NO_PIP_SUPPORT
   // Change Pip source to be the same as Main if there is a conflict
   if (!IsMainSourceValid(UserPrefCurrentInputMain))
   {
  
#if 0//def PIP_PORT_INDEPENDENCE
      DisablePipModeDetection(); // Disable PIP mode detection if port collision
#else
   ...
   if (gB_CurrentInputConnector[gmvb_MainChannel] != ALL_BOARDS_DP1)
	temp=ALL_BOARDS_DP1;
   else	
	temp=ALL_BOARDS_CVBS1;
//if(UserPrefCurrentInputMain == UserPrefCurrentInputPip)
  SelectInput(gmvb_PipChannel, temp, NORMAL_MODE);




   //ResourceResolver();
   #if 0//(DISPLAY_FUNCTION==VGA_VIDEO)
...
if(UserPrefVIDEO_D==1)
{
     ...
     if (UserPrefCurrentInputMain == ALL_BOARDS_VGA1)
		...
		if((UserPrefCurrentInputPip != ALL_BOARDS_CVBS1)&&(UserPrefCurrentInputPip != ALL_BOARDS_SVIDEO1))
		UserPrefCurrentInputPip = ALL_BOARDS_CVBS1;
...
//	if(UserPrefPipMode == NO_PIP) //LEO DEL
//		SelectInput(gmvb_PipChannel, UserPrefCurrentInputPip, NORMAL_MODE);	//LEO DEL
   ...
   gm_ClearRegBitsWord(DP12RX1_LINK_CONTROL, DPRX_LANE_POLARITY);
   gm_SetRegBitsWord(DP12RX1_LINK_CONTROL, DPRX_SWAP_LANE);
   #endif
...
void AdjustCurrentInputPip(void)
{
   ...
   // 20090411 #1, Added flag for change port wake up for DP.
#if 1
   if (UserPrefCurrentInputPip == ALL_BOARDS_DP1)
   {
      if (NonDPPortSleep)
      {
         DpRxSendHpdLongPulse();
...
static void CopyUserPrefs(gmt_PHY_CH B_Channel)
{
   if (B_Channel == gmvb_MainChannel)
   {
      ...
      //UserPrefHSyncDelay = UserPrefHSyncDelay_PIP;     // Test 20121218
